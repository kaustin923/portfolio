...
export const EduShrinkScene = ({duration}: {duration: number}) => {
  ...
  const x = interpolate(compress, [0,.32,.65,1], [120,260,505,750]);
  const y = interpolate(compress, [0,.32,.65,1], [500,610,760,830]);
  const scale = interpolate(compress, [0,.32,.65,1], [1,.72,.43,.19]);
  return (
    <EduScene>
      <EduMasthead section="03 / MODEL COMPRESSION" />
      <div style={{position: 'absolute', top: 330, left: 96, right: 96, fontFamily: TOKENS.font.display, fontSize: 64, lineHeight: .92, letterSpacing: '-.045em'}}>SAME ABILITY.<br/><span style={{color: TOKENS.color.pending}}>A FRACTION OF THE SIZE.</span></div>
      ...
      <div style={{position: 'absolute', left: 580, top: 570}}>
        <PhoneFrame width={340} height={590} glow={settle > .1}>
          <div style={{position: 'absolute', left: 80, top: 170, width: 180, height: 180, border: `7px solid ${TOKENS.color.pending}`, boxSizing: 'border-box', display: 'grid', placeItems: 'center', boxShadow: `0 0 ${settle * 55}px rgba(217,164,65,.7), inset 0 0 ${settle * 38}px rgba(217,164,65,.3)`}}>
            <div style={{fontFamily: TOKENS.font.mono, color: TOKENS.color.bone, fontSize: 22, fontWeight: 700, textAlign: 'center'}}>SPEECH<br/>AI</div>
          </div>
      ...
      <div style={{position: 'absolute', left: x, top: y, transform: `translate(-50%,-50%) scale(${scale})`, transformOrigin: 'center'}}><ServerBlock pulse={(frame % 30) / 30} /></div>
