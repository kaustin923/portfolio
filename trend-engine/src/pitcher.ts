...
Each pitch also needs: a curiosity-gap headline (≤9 words, stops a scroll as silent text AND implies the personal stakes), an unexpected angle (a real insight or twist, not just coverage), and a vertical tag (finance/sports/tech/culture).

STRONG BIAS TOWARD SPORTS & FANTASY: sports — especially fantasy football, NFL, and anything with a betting/drafting/prediction edge — is the priority lane (huge, hungry, always-on audience). At least half of every batch should be sports/fantasy. Prefer these over cold business angles.

MUST BE NON-OBVIOUS: the insight can't be something the audience already knows or could guess — no "these are the top players", no obvious takes. It's the hidden tell, the counterintuitive angle, the thing casual fans miss ("the empty backfield job nobody's drafting", "the preseason stat that predicts busts"). If a knowledgeable fan would say "yeah, obviously" — kill it.

MUST BE EXPLAIN-LIKE-I'M-5 SIMPLE: one clear idea per pitch, explainable in a sentence a total newcomer gets instantly. No dense stat-chains, no jargon, no confusing number relationships (the audience will tune out). Simple + non-obvious is the target. eventPeg must be null unless pegged to a named event with an explicit full date; when present use "<event name> on YYYY-MM-DD". Never use "upcoming", "date pending", an unknown date, or a past event as a future peg. Pitches must be DIVERSE (no two same vertical+format) and NEVER generic ('X explained' headlines auto-rejected — regenerate once).`;
